<div class="min-h-screen bg-black text-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        <div class="text-center mb-12">
            ...
            <p class="text-gray-400 text-lg">Calcula aranceles, IVA y costos aduaneros</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white/5 backdrop-blur-xl border border-yellow-500/30 p-6 rounded-lg hover:border-yellow-500 transition-all">
                    <label class="block text-yellow-500 font-bold mb-4 text-sm uppercase tracking-wide">Categoría de Producto</label>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <button wire:click="$set('categoria', 'general')"
                            class="px-4 py-4 border-2 rounded-lg font-bold transition-all transform hover:scale-105 {{ $categoria === 'general' ? 'border-yellow-500 bg-yellow-500 text-black shadow-lg shadow-yellow-500/50' : 'border-yellow-500/30 text-yellow-400 hover:border-yellow-500' }}">
                            GENERAL<span class="block text-xs font-normal mt-1 {{ $categoria === 'general' ? 'text-black/70' : 'text-gray-500' }}">10% Arancel</span>
                        </button>
                        <button wire:click="$set('categoria', 'alimentos')"
                            class="px-4 py-4 border-2 rounded-lg font-bold transition-all transform hover:scale-105 {{ $categoria === 'alimentos' ? 'border-yellow-500 bg-yellow-500 text-black shadow-lg shadow-yellow-500/50' : 'border-yellow-500/30 text-yellow-400 hover:border-yellow-500' }}">
                            ALIMENTOS<span class="block text-xs font-normal mt-1 {{ $categoria === 'alimentos' ? 'text-black/70' : 'text-gray-500' }}">5% Arancel</span>
                        </button>
                        <button wire:click="$set('categoria', 'tecnologia')"
                            class="px-4 py-4 border-2 rounded-lg font-bold transition-all transform hover:scale-105 {{ $categoria === 'tecnologia' ? 'border-yellow-500 bg-yellow-500 text-black shadow-lg shadow-yellow-500/50' : 'border-yellow-500/30 text-yellow-400 hover:border-yellow-500' }}">
                            TECNOLOGÍA<span class="block text-xs font-normal mt-1 {{ $categoria === 'tecnologia' ? 'text-black/70' : 'text-gray-500' }}">15% Arancel</span>
                        </button>
                        <button wire:click="$set('categoria', 'textil')"
                            class="px-4 py-4 border-2 rounded-lg font-bold transition-all transform hover:scale-105 {{ $categoria === 'textil' ? 'border-yellow-500 bg-yellow-500 text-black shadow-lg shadow-yellow-500/50' : 'border-yellow-500/30 text-yellow-400 hover:border-yellow-500' }}">
                            TEXTIL<span class="block text-xs font-normal mt-1 {{ $categoria === 'textil' ? 'text-black/70' : 'text-gray-500' }}">20% Arancel</span>
                        </button>
                    </div>
                </div>

                <div class="bg-white/5 backdrop-blur-xl border border-yellow-500/30 p-6 rounded-lg hover:border-yellow-500 transition-all">
                    <label class="block text-yellow-500 font-bold mb-2 text-sm uppercase tracking-wide">País de Origen</label>
                ...
                <div class="bg-white/5 backdrop-blur-xl border border-yellow-500/30 p-6 rounded-lg hover:border-yellow-500 transition-all">
                    <h3 class="text-yellow-500 font-bold mb-4 text-sm uppercase tracking-wide">Valores (FOB + Flete + Seguro = CIF)</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-gray-400 mb-2 text-xs">Valor Mercancía (FOB) *</label>
                            <div class="relative">
                                <span class="absolute left-4 top-3 text-yellow-500 font-bold">$</span>
                                <input type="number" wire:model="valorMercancia" step="0.01" placeholder="10000"
                                    class="w-full bg-black/50 border border-yellow-500/30 text-white pl-8 pr-4 py-3 focus:outline-none focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/50 transition-all rounded-lg" required>
                            </div>
                        </div>
                        <div>
                            <label class="block text-gray-400 mb-2 text-xs">Valor Flete</label>
                            <div class="relative">
                                <span class="absolute left-4 top-3 text-yellow-500 font-bold">$</span>
                                <input type="number" wire:model="valorFlete" step="0.01" placeholder="500"
                                    class="w-full bg-black/50 border border-yellow-500/30 text-white pl-8 pr-4 py-3 focus:outline-none focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/50 transition-all rounded-lg">
                            </div>
                        </div>
                        <div>
                            <label class="block text-gray-400 mb-2 text-xs">Valor Seguro</label>
                            <div class="relative">
                                <span class="absolute left-4 top-3 text-yellow-500 font-bold">$</span>
                ...
                <div class="bg-yellow-500/10 border border-yellow-500 p-6 rounded-lg">
                    <h3 class="text-yellow-400 font-bold mb-3 uppercase text-sm">Fórmula CIF (Cost, Insurance, Freight)</h3>
                ...
                <div class="bg-white/5 backdrop-blur-xl border border-yellow-500/30 p-6 rounded-lg sticky top-24">
                    ...
                    @if ($resultado !== null)
                        <div class="bg-yellow-500 text-black p-8 rounded-lg mb-6 text-center transform hover:scale-105 transition-transform">
                            <p class="text-sm font-bold mb-2 uppercase tracking-wide">Total Impuestos</p>
                            ...
                            <div class="space-y-3">
                                ...
                                @foreach ($desglose as $concepto => $valor)
                                    <div class="flex justify-between items-center py-2 border-b border-yellow-500/20 hover:border-yellow-500/50 transition-colors">
                                        <span class="text-gray-300 text-sm {{ str_contains($concepto, 'Total') ? 'font-bold' : '' }}">{{ $concepto }}</span>
                                        <span class="font-bold {{ str_contains($concepto, 'Total') ? 'text-yellow-500' : 'text-white' }}">
                                            @if (is_numeric($valor))
                                                {{ $valor }}
                                            @else
                                                ${{ $valor }}
        ...
        <div class="mt-8 bg-yellow-500/10 border-l-4 border-yellow-500 p-6 rounded">
            <div class="flex items-start">
                <svg class="w-6 h-6 text-yellow-500 mr-4 shrink-0 mt-1" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                </svg>
                <div>
                    <p class="text-yellow-400 font-bold mb-1">Nota Importante</p>
                    <p class="text-gray-400 text-sm">Las tasas de impuestos varían según el país y tipo de producto. Esta calculadora usa tasas estimadas para Argentina. Consulta con un agente aduanal para valores exactos.</p>
                </div>
            </div>
        </div>
    </div>
